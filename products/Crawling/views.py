...
def goat_collections(request):
    ...
    for j in range(7):
        for i in range(1, 200):
            ...
            url_list_name = ['popular_release', 'recent_kicks', 'recent_apparel', 'newe_in', 'just_drop', 'womens',
                             'toddler', 'most_wanted', ]
...
def create_new_kick_data(products_list, p, brand):
    ...
    try:
        ...
        kick = kicks(
            ...
            category=kream_category if kream_category else products_list[p]['data'].get('category'),
...
def sneaker_img_paser(request):
    ...
    all_products = kicks.objects.filter(local_imageUrl='media/images/defaultImg.png').exclude(
        imageUrl__isnull=True).order_by('-releaseDate')
    ...
    for i, p in enumerate(all_products):
        ...
        # Stockx url 일 경우, 
            ...
            try:
                req.urlretrieve(imageUrl,
                                '/var/services/web/kickin/media/images/sneakers/' + file_name[:file_name.find('?')])
                # 해당 제품 db 업데이트
