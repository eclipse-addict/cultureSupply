    ...
    for j in range(7):
        for i in range(1, 200):
            ...
            if not products_list:
                print(
                    f"No more products on {url_list_name[j]} found finished at page : {i}page."
                )
                break
        ...
        if kick.local_imageUrl != "media/images/defaultImg.png":  # 기존 이미지가 존재 하는 경우
            ...
            for type in img_type_list:
                ...
                img = productImg(  # left, back, top, bottom, additional 이미지 저장
                    product=kick, img_url="media/images/defaultImg.png", type=type
                )
                img.save()  # db에 저장
...
def dev_crawling_test(request):
    ...
    return JsonResponse(
        {"new_product_count": 100, "time": "300s"}, status=status.HTTP_200_OK
    )
